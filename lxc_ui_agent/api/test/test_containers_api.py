...
class TestContainersAPI(unittest.TestCase):
    def setUp(self):
        ...
        # replace the default app with one with mock binaries
        self.app.application.config["singletons"]["containers"] = lxc.test.helpers.get_containers_with_mock_binaries()

    def tearDown(self):
        lxc.test.helpers.cleanup_containers_with_mock_binaries(self.app.application.config["singletons"]["containers"])
